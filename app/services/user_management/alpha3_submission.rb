  ...
  class Alpha3Submission < ServicesBase
    ...
    def validate_and_sanitize
      ...
      if !is_valid_url?(@video_url)
        error_data[:video_url] = 'Video url is not valid'
      end

      if !is_valid_url?(@url_blog)
        error_data[:url_blog] = 'Blog url is not valid'
      end

      if !is_valid_url?(@tech_doc)
        error_data[:tech_doc] = 'Tech doc url is not valid'
      end

      if !is_valid_url?(@project_url)
        error_data[:project_url] = 'Project url is not valid'
      end
