namespace :onetimer do
  ...
  task :get_st_bonus_detail_report => :environment do
    ...
    community_bonus = if total_st_sold >= 180000000
                        30
                      elsif total_st_sold >= 120000000
                        25
                      elsif total_st_sold >= 100000000
                        20
                      else
                        0
                      end
    ...
    csv_data = []
    csv_data << ['ethereum_address', 'purchase_in_st_wei', 'purchase_in_st_basic_unit' 'st_bonus_wei_value', 'st_bonus_value_in_basic_unit', 'pos_bonus', 'community_bonus']

    total_bonus_in_st_wei = 0
    st_to_user_mapping.each do |ethereum_address, purchase_in_st_wei|
